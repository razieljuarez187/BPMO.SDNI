...
namespace BPMO.SDNI.Contratos.FSL.RPT
{
	public partial class PagareRPT : DevExpress.XtraReports.UI.XtraReport
	{
		...
		private const string xmlFirmas = "BPMO.SDNI.Reportes.Firmas.xml";
		private const string xmlPagare = "BPMO.SDNI.Reportes.Contrato.FSL.Pagare.xml";
		...
		public void ImprimirPagare()
		{
			...
			if(datosPagare.ContainsKey("Plazo"))
				plazo = (int) datosPagare["Plazo"];

			if (datosPagare.ContainsKey("Mensualidad"))
				mensualidad = (decimal) datosPagare["Mensualidad"];
			mensualidad = Math.Round(mensualidad, 2);

			string sCantidad = string.Format("$ {0:#,##0.00}", cantidad);
			string sCantidadLetras = enletras(cantidad, sMoneda,true );
			string sPlazo = enletras(plazo,"",false);
			string sMensualidad = enletras(mensualidad, sMoneda, true);
			...
			lblBuenoPor.Html = textoPagare.ToString().ToUpper();
			#endregion
			#region Contenido
			...
			textoPagare.Replace("[PLAZO]", plazo.ToString().ToUpper());
			textoPagare.Replace("[PLAZO_LETRAS]", sPlazo.ToUpper());
			textoPagare.Replace("[MENSUALIDAD]", string.Format("$ {0:#,##0.00}",mensualidad).ToUpper());
			textoPagare.Replace("[MENSUALIDAD_LETRAS]", sMensualidad.ToUpper());
			...
			if (datosPagare.ContainsKey("FechaInicio"))
			{
				...
				textoPagare.Replace("[MES_INICIO]", string.Format("{0:MMMM DE yyyy}", fechaInicio).ToUpper());
			}
		...
		public string enletras(decimal num, string moneda, bool conDecimales)
		{
			string res, dec = "";
			Int64 entero;
			int decimales;
        ...
        private static string ATexto(decimal value)
        {
            ...
            else if (value < 1000000) Num2Text = ATexto(Math.Truncate(value / 1000)) + " MIL " + (((value%1000) > 0) ? ATexto(value % 1000) : string.Empty); //SC_0051 if ((value%1000) > 0), originalmente solo tenia un if al principio
            ...
            else if (value < 1000000000000)
            {
                ...
                if ((value - Math.Truncate(value/1000000)*1000000) > 0)
                    Num2Text = Num2Text + " " + ATexto(value - Math.Truncate(value/1000000)*1000000);
            }
            else if (value == 1000000000000) Num2Text = "UN BILLON";
            else if (value < 2000000000000)
                Num2Text = "UN BILLON " + ATexto(value - Math.Truncate(value/1000000000000)*1000000000000);
            else
            {
                Num2Text = ATexto(Math.Truncate(value/1000000000000)) + " BILLONES";

                if ((value - Math.Truncate(value/1000000000000)*1000000000000) > 0)
                    Num2Text = Num2Text + " " + ATexto(value - Math.Truncate(value/1000000000000)*1000000000000);
            }
            return Num2Text;
