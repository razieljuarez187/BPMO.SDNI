//Esta clase satisface los requerimientos del CU018 - Imprimir contrato maestro
using System;
...
namespace BPMO.SDNI.Contratos.FSL.RPT {
	public partial class ContratoMaestroRPT : DevExpress.XtraReports.UI.XtraReport {
		...
		public static string NombreArchivo = "BPMO.SDNI.Reportes.Contrato.Fisica.xml";
		...
		public ContratoMaestroRPT() {
			try {
				InitializeComponent();
			} catch { throw; }
		}

		public ContratoMaestroRPT(Dictionary<string, object> ht, string url) {
			try {
				InitializeComponent();
				if (!ht.ContainsKey("PersonaFisica"))
					throw new Exception("Es necesario especificar el tipo de regimén del cliente al que se le esta generando el contrato");
		...
		private void ImprimirPersona(Dictionary<string, object> ht) {
			...
			path += "BPMO.SDNI.Reportes.Contrato.Maestro.xml";
			ContratoFSLBO contrato = (ContratoFSLBO)ht["Contrato"];
			...
			if (File.Exists(path)) {
							 ...
							 select new
								 {
									 ...
									 Leyenda = new
										 {
											 ...
											 Depositarios = (from ldep in xmlDoc.Descendants("leyenda").Descendants("opcionales")
															 select ldep.Element("depositarios")).FirstOrDefault()
										 },
									 ...
									 ContenidoDeclaraciones = from cd in xmlDoc.Descendants("declaraciones").Descendants("contenido")
															  select new
															  {
																  ...
																  ObligadosDepositarios = new
																  {
																	  ...
																	  ClausulasD = (from dosdd in xmlDoc.Descendants("declaraciones").Descendants("contenido").Descendants("obligadosDepositarios").Descendants("declaracionOS").Where
																									   (dosdd => (string)dosdd.Attribute("grupo") == "D")
																					select new
																						{
																							Valor = dosdd.Element("valor").Value,
																							Tipo = dosdd.Element("tipo").Value
																						}).FirstOrDefault(),
																	  ClausulasE = (from dosde in xmlDoc.Descendants("declaraciones").Descendants("contenido").Descendants("obligadosDepositarios").Descendants("declaracionOS").Where
																									   (dosde => (string)dosde.Attribute("grupo") == "E")
																					select new
																					{
																						Valor = dosde.Element("valor").Value,
																						Tipo = dosde.Element("tipo").Value
																					}).FirstOrDefault()
																  },
																  Definiciones = cd.Element("definiciones").Value,
									 ...
									 ContenidoClausulas = from pci in xmlDoc.Descendants("clausulas").Descendants("contenido").Descendants("clausula")
														  select new
														  {
															  ...
															  ContenidoOpcional = from pcvo in xmlDoc.Descendants("clausulas").Descendants("opcionales").Descendants("opcion")
																				  select new
																				  {
																					  Valor = pcvo.Element("valor").Value,
																					  Clausula = pcvo.Element("clausula").Value,
																					  ID = pcvo.Element("id").Value
																				  }
														  },
				...
				foreach (var rep in titulo) {
					...
					foreach (var cde in rep.ContenidoDeclaraciones) {
						...
						if (contrato.Cliente != null) {
							...
							if (contrato.Cliente.EsFisico.Value) {
											...
											if (mismaActa) {
												...
												foreach (var clausulasa in cde.ObligadosDepositarios.ClausulasB) {
													...
													if (((ObligadoSolidarioMoralBO)osm).Representantes.Count > 1) {
														...
														elementos.Replace("[ACTAS_COMOP_REP_OS]", this.ObtenerInformacionAdicionalActas(repsOS, cde.ObligadosDepositarios.ClausulasE.Valor, false));
													} else {
								...
								elementos.Replace("[DEPOSITARIOS]", contrato.Cliente.Nombre);
								elementos.Replace("[DEPO_PLURAL]", string.Empty);
								elementos.Append(pdhhijo);
								elementos.Replace("[TEXTO]", cde.ObligadosDepositarios.ClausulasD.Valor);
								elementos.Replace("[DEPO_PLURAL]", string.Empty);
								#endregion
							} else {
								...
								if (contrato.SoloRepresentantes.HasValue) {
									if (contrato.SoloRepresentantes.Value) {
												...
												foreach (var osm in listaObligadosMorales) {
													...
													if (mismaActa) {
														...
														if (((ObligadoSolidarioMoralBO)osm).Representantes.Count > 1) {
															...
															elementos.Replace("[ACTAS_COMOP_REP_OS]", this.ObtenerInformacionAdicionalActas(representantesOS, cde.ObligadosDepositarios.ClausulasE.Valor, false));
										...
										if (contrato.RepresentantesLegales != null) {
											if (contrato.RepresentantesLegales.Count > 0) {
												#region Depositarios
												var depositarios = contrato.RepresentantesLegales.FindAll(x => ((RepresentanteLegalBO)x).EsDepositario.Value == true);
												if (depositarios != null) {
													elementos.Replace("[DEPO_PLURAL]", depositarios.Count > 1 ? "s" : string.Empty);
													elementos.Replace("[ARTICULO_DEP]", depositarios.Count > 1 ? "los" : "[ARTICULO_DEP]");
													elementos.Replace("[DEPOSITARIOS]", depositarios.Count > 0 ? this.ObtenerNombreDepositarios(depositarios.ConvertAll(x => (RepresentanteLegalBO)x)) : string.Empty);
												} else elementos.Replace("[DEPO_PLURAL]", string.Empty);
												#endregion
											} else elementos.Replace("[DEPO_PLURAL]", string.Empty);
										} else elementos.Replace("[DEPO_PLURAL]", string.Empty);
										#endregion
						...
						#endregion
						#region Definiciones
						html.Replace("[DEFINICIONES]", cde.Definiciones);
						#endregion
						#region EClausulas
					...
					foreach (var cc in rep.ContenidoClausulas) {
						...
						if (cc.ID.CompareTo("5") == 0) {
							foreach (var iopcional in cc.ContenidoOpcional) {
								if (iopcional.Clausula.CompareTo("5") == 0 && iopcional.ID.CompareTo("PUNTO_OBLIGADOS") == 0) {
									if (contrato.SoloRepresentantes.HasValue) {
										if (contrato.SoloRepresentantes.Value)
											item.Replace("[PUNTO_OBLIGADOS]", cc.ContenidoOpcional.ToList().FirstOrDefault(o => o.Clausula == "5.1" && o.ID == "NA_PUNTO_OBLIGADOS").Valor);
										else item.Replace("[PUNTO_OBLIGADOS]", iopcional.Valor);
										if (contrato.ObligadosSolidarios != null) {
											if (contrato.ObligadosSolidarios.Count > 0)
												item.Replace("[OBLIGADOS_SOLIDARIOS]", this.ObtenerNombresObligadosSolidarios(contrato.ObligadosSolidarios));
											else if (contrato.RepresentantesLegales != null)
												item.Replace("[OBLIGADOS_SOLIDARIOS]", this.ObtenerNombreRepresentantesLegales(contrato.RepresentantesLegales));
										}
									}
								}
							}
						...
						if (cc.ID.CompareTo("15") == 0) {
							if (contrato.SoloRepresentantes.HasValue) {
								if (contrato.SoloRepresentantes.Value) {
									var adicional = cc.ContenidoOpcional.ToList().FirstOrDefault(o => o.Clausula == "15.1" && o.ID == "NA_DEPOSITARIA");
									item.Replace("[DEPOSITARIA]", adicional.Valor);
								} else {
									var adicional = cc.ContenidoOpcional.FirstOrDefault(o => o.Clausula == "15" && o.ID == "DEPOSITARIA");
									item.Replace("[DEPOSITARIA]", adicional.Valor);
								}
							}
					...
					if (contrato.DiasPago.HasValue)
						html.Replace("[DIAS_PRONTO_PAGO]", contrato.DiasPago.Value.ToString());
					if (contrato.PorcentajePenalizacion.HasValue)
						html.Replace("[PORCENTAJE_PENALIZACION]", Math.Round(contrato.PorcentajePenalizacion.Value, 2).ToString());

					if (contrato.Cliente != null) {
						if (contrato.Cliente.EsFisico.HasValue) {
							if (contrato.Cliente.EsFisico.Value) {
								html.Replace("[DEPO_PLURAL]", string.Empty);
								html.Replace("[ART_DEPO]", "EL");
								html.Replace("[PREP_DEPO]", string.Empty);
								html.Replace("[DEPO_PLU_MIN]", string.Empty);
								html.Replace("[DEPOSITARIOS]", contrato.Cliente.Nombre);
							} else {
								if (contrato.RepresentantesLegales != null) {
									if (contrato.RepresentantesLegales.Count > 0) {
										var reps = contrato.RepresentantesLegales.ConvertAll(x => (RepresentanteLegalBO)x);
										var deps = reps.FindAll(x => x.EsDepositario.Value);
										html.Replace("[DEPO_PLURAL]", deps.Count > 1 ? "S" : string.Empty);
										html.Replace("[ART_DEPO]", deps.Count > 1 ? "LOS" : "EL");
										html.Replace("[PREP_DEPO]", deps.Count > 1 ? "n" : string.Empty);
										html.Replace("[DEPO_PLU_MIN]", deps.Count > 1 ? "s" : string.Empty);
										html.Replace("[DEPOSITARIOS]", deps.Count > 0 ? this.ObtenerNombreDepositarios(deps) : string.Empty);
									} else {
										html.Replace("[DEPO_PLURAL]", string.Empty);
										html.Replace("[ART_DEPO]", "EL");
										html.Replace("[PREP_DEPO]", string.Empty);
					...
					if (contrato.Cliente != null) {
						if (contrato.Cliente.EsFisico.HasValue) {
							if (contrato.Cliente.EsFisico.Value) {
								...
								html.Replace("[LEYENDA_DEPOSITARIOS]", rep.Leyenda.Depositarios.Value);
							} else {
								if (contrato.SoloRepresentantes.HasValue) {
									if (!contrato.SoloRepresentantes.Value) {
										html.Replace("[LEYENDA_OBLIGADOS]", rep.Leyenda.Obligados.Value);
										...
										html.Replace("[LEYENDA_DEPOSITARIOS]", string.Empty);
									}
								} else {
									html.Replace("[LEYENDA_OBLIGADOS]", rep.Leyenda.Obligados.Value);
									html.Replace("[LEYENDA_DEPOSITARIOS]", rep.Leyenda.Depositarios.Value);
						...
						if (contrato.Cliente.EsFisico.HasValue) {
							if (contrato.Cliente.EsFisico.Value) {
								if (!string.IsNullOrEmpty(contrato.Cliente.Nombre) &&
									!string.IsNullOrWhiteSpace(contrato.Cliente.Nombre)) {
									html.Replace("[DEPO_PLURAL]", string.Empty);
									html.Replace("[DEPOSITARIOS]", contrato.Cliente.Nombre);
								}
							} else {
								List<RepresentanteLegalBO> repLegales = contrato.RepresentantesLegales.ConvertAll(x => (RepresentanteLegalBO)x);
								if (repLegales != null) {
									if (repLegales.Count > 0) {
										#region Depositarios

										var depositarios = repLegales.FindAll(x => x.EsDepositario.Value == true);
										if (depositarios != null) {
											html.Replace("[DEPO_PLURAL]", depositarios.Count > 1 ? "s" : string.Empty);
											html.Replace("[DEPOSITARIOS]",
														 this.ObtenerNombreDepositarios(
															 repLegales.FindAll(x => x.EsDepositario.Value)));
										} else html.Replace("[DEPO_PLURAL]", string.Empty);
										#endregion
									} else html.Replace("[DEPO_PLURAL]", string.Empty);
								} else {
									if (!string.IsNullOrEmpty(contrato.Cliente.Nombre) &&
		...
		private string ObtenerNombreDepositarios(List<RepresentanteLegalBO> deps) {
			StringBuilder nombres = new StringBuilder();
			foreach (var rep in deps) {
				nombres.Append(", " + rep.Nombre);
			}
			if (nombres.Length == 0)
				return string.Empty;
		...
		private string ObtenerNotificantes(string p, ContratoFSLBO contrato) {
			...
			if (contrato.ObligadosSolidarios != null) {
				...
				if (!ObligadosTemp.Any() && contrato.RepresentantesLegales != null && !contrato.SoloRepresentantes.Value) {
					foreach (var repT in contrato.RepresentantesLegales) {
						ObligadoSolidarioFisicoBO osfT = new ObligadoSolidarioFisicoBO();
						osfT.Nombre = repT.Nombre;
						osfT.Activo = true;
						osfT.ActaConstitutiva = repT.ActaConstitutiva;
						osfT.DireccionPersona = repT.DireccionPersona;
						personasOS.Add(osfT);
					}
				}
				...
				if (contrato.Cliente != null) {
					if (contrato.Cliente.EsFisico.HasValue) {
						if (contrato.Cliente.EsFisico.Value) {
							RepresentanteLegalBO depTemp = new RepresentanteLegalBO();
							depTemp.EsDepositario = true;
							depTemp.Nombre = contrato.Cliente.Nombre;
							DireccionClienteBO dir = contrato.Cliente.Direcciones.FirstOrDefault(dirf => dirf.Primaria.Value);
							depTemp.DireccionPersona = new DireccionPersonaBO();
							depTemp.DireccionPersona.Calle = dir.Direccion;
							depTemp.DireccionPersona.CodigoPostal = dir.CodigoPostal;
							depTemp.DireccionPersona.Colonia = dir.Colonia;
							depTemp.DireccionPersona.Ubicacion = dir.Ubicacion;
							personasOS.Add(depTemp);
						} else {
							if (contrato.RepresentantesLegales != null) {
								if (contrato.RepresentantesLegales.Count > 0) {
									personasOS.AddRange(contrato.RepresentantesLegales.FindAll(x => ((RepresentanteLegalBO)x).EsDepositario.Value));
								}
							}
						}
					}
						...
						if (personaIzquierda == null) //Si persona Izquierda es null
		    {
							...
							if (personaIzquierda.TipoPersona.Value == ETipoPersona.ObligadoSolidario) {
								...
								if (repIzq.EsDepositario != null && repIzq.EsDepositario == true) {
									lineaNotificante.Replace("[TITLE_LEFTH]", "<b>DEPOSITARIO:</b>");
									lineaNotificante.Replace("[NAME_LEFTH]", repIzq.Nombre);
									lineaNotificante.Replace("[NAME_REP_LEFTH]", repIzq.Direccion);
									lineaNotificante.Replace("[DIREC_REP_LEFTH]", string.Empty);
								} else {
									lineaNotificante.Replace("[TITLE_LEFTH]", "<b>OBLIGADO SOLIDARIO:</b>");
							...
							if (personaDerecha == null) //Si persona Derecha es null
		    {
								...
								if (personaDerecha.TipoPersona.Value == ETipoPersona.ObligadoSolidario) {
									...
									if (repDer.EsDepositario != null && repDer.EsDepositario == true) {
										lineaNotificante.Replace("[TITLE_RIGTH]", "<b>DEPOSITARIO:</b>");
										lineaNotificante.Replace("[NAME_RIGTH]", repDer.Nombre);
										lineaNotificante.Replace("[NAME_REP_RIGTH]", repDer.Direccion);
										lineaNotificante.Replace("[DIREC_REP_RIGTH]", string.Empty);
									} else {
										lineaNotificante.Replace("[TITLE_RIGTH]", "<b>OBLIGADO SOLIDARIO:</b>");
		...
		public string ObtenerLeyendaFirmas(ContratoFSLBO contrato) {
			...
			if (esFisico) {
				...
				if (contrato.RepresentantesLegales != null) {
					if (contrato.RepresentantesLegales.Count > 0) {
						...
						var depositarios = contrato.RepresentantesLegales.FindAll(x => ((RepresentanteLegalBO)x).EsDepositario.Value);
						if (depositarios.Count > 1)
							leyendaFirma.Replace("[S_DEPOSITARIO]", "S");
						else
							leyendaFirma.Replace("[S_DEPOSITARIO]", string.Empty);
						leyendaFirma.Replace("[REPRESENTANTES_LEGALES_COMO_DEPOSITARIOS]", this.ObtenerNombreDepositarios(depositarios.ConvertAll(x => (RepresentanteLegalBO)x)));
					}
				}
			}
		...
		protected void ProcesaFirmas(List<Dictionary<string, string>> firmas, ContratoFSLBO contrato) {
		...
		protected void SetFirma(string htmlFirmas, bool firstElement, bool lastElement, ContratoFSLBO contrato) {
			CreateDocument();
